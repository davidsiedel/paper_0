...
\subsection{Elementary residual and its derivatives}

\paragraph{Internal forces} The internal forces vector depends on the product of the stress values with the gradient operator computed at quadrature points, plus a supplementary force corresponding to that of the traction between the cell and its boundary
...
The external forces vector is the evaluation of the given bulk and boundary loads at respective cell and face quadrature points tested against the respective cell and face shape functions, and is denoted
%
%
%
\begin{equation}
    \label{hho_incremenaljjjdjj2}
    \begin{aligned}
        \begin{Bmatrix}
            F_{T}^{ext}
        \end{Bmatrix}
...
\paragraph{Residual} Following \eqref{eq_residual_def}, the elementary residual vector writes

%
%
%
\begin{equation}
    \label{hho_incremenaljjjdjj3}
    \begin{aligned}
        \begin{Bmatrix}
            R_{T}
            (
...
The tangent matrix $[K_{\cell}(\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})]$ expresses the derivative of $\{ R_{T} ( \tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k} \}$ with respect to $\{ \tensori{u}{}_{\cell}^{l} \}$ and $\{ \tensori{u}{}_{\dCell}^{k} \}$, and writes as the product of the gradient operators by the tangent operators $\tensoriv{A}{}(\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})$ at quadrature points, plus the additional stabilization term such that
%
%
%
\begin{equation}
    ...
    \begin{aligned}
        ...
        %     (\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})
        %     &&
...
%
%
where the tangent operator $\tensoriv{A}{}(\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})$ is the derivative of the stress with respect to the displacement gradient
%
%
%
\begin{equation}
...
%
%
and the tangent matrix can be decomposed by blocks such that
%
%
%
\begin{equation}
    ...
    \begin{aligned}
        ...
        =
        \begin{bmatrix}
            % K_{\cell \cell} (\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})
            \partial R_{\cell}^{\cell} / \partial \tensori{u}{}_{\cell}^{l}
            &&
            % K_{\cell \dCell} (\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})
            \partial R_{\dCell}^{\cell} / \partial \tensori{u}{}_{\cell}^{l}
            \\
            % K_{\dCell \cell} (\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})
            \partial R_{\cell}^{\cell} / \partial \tensori{u}{}_{\dCell}^{k}
            &&
            % K_{\dCell \dCell} (\tensori{u}{}_{\cell}^{l}, \tensori{u}{}_{\dCell}^{k})
            \partial R_{\dCell}^{\cell} / \partial \tensori{u}{}_{\dCell}^{k}
        \end{bmatrix}
    \end{aligned}
\end{equation}
%
%
%
where one retrieves the expressions of the residual derivatives as introduced in Section \ref{sec_cell_u_elmini}
...
% However, it allows to consider extending the present non-linear algorithm to \textit{e.g.} constrained resolution algorithm, to solve inequality constrained problems, as encountered in multi-field plasticity \cite{schroder_small_2015} for instance.
