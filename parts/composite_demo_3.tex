...
In this section one considers a subpart \(T\) of the body
\(\bodyLag\). This subpart is in equilibrium with the rest of the body
\(\Omega\backslash T\) if the displacements and the normal traction are
continuous at the boundary of \(\partial T\).
...
On the contrary, this condition can be weakened by introducting an
elastic interface of negligible size between \(T\) and
\(\Omega\backslash T\). This is the basis of Discontinuous Galerkin
methods.

In this paper, we consider HDG and HHO methods where two elastic
...
HDG/HHO methods, namely the reconstructed gradient and the stabilisation
operator.
