...
\newcommand\dirichletBodyEul{\partial_d \Omega_t}
\newcommand\neumannBodyEul{\partial_n \Omega_t}
\newcommand\dirichletBodyLag{\partial_D \Omega}
\newcommand\neumannBodyLag{\partial_N \Omega}
...
\begin{frontmatter}
  ...
  \begin{abstract}
    ...
    discretization method which has only been recently applied from to
    non linear computational mechanics.

    The Hybrid High Order method divides the domain of interest
    in cells of abitrary polyhedral shape, whose boundaries is called
    the skeleton, and introduces two kinds of degrees of freedom: the
    displacements in the cell and the displacements of the skeleton.

    Most introductory materials to the HHO method is focus on
    the mathematical aspects of the methods. While those aspects are
    ...
    classical Hu–Washizu functional and discusses the implementation of
    the method using notations closed to the one used in standard finite
    elements textbooks. From the point of view of numerical
    performances, the elimination of the cell degrees of freedom is
    mandory to to reduce the size of the stiffness matrix of the system.
    The standard static condensation, is presented, as well as a novel
    strategy called "cell equilibrium". Avantages and disadvantages of
    both strategies are discussed.
    
    The resolution of axisymmetrical problems, which has seldom,
    if ever, been discussed in the litterature, is then presented.
  ...
  \begin{keyword}
    ...
    method \sep Volumetric locking \sep Static condensation \sep
  \end{keyword}
...
\input{parts/axisymmetric.tex}

\section{Implementation}
\input{parts/implementation.tex}

\section{Numerical exemples}
